<template>
    <div class="app-container">
                        ...
                        >
                            <template slot-scope="scope">
                                <el-switch
                                    ...
                                    :disabled="$hasBP('bnt.patient.update') === false"
                                    @change="authPatient(scope.row.id)"
                                >
                                </el-switch>
                            </template>
                        </el-table-column>
                        <el-table-column label="操作" align="center" fixed="right">
                            <template slot-scope="scope">
                                ...
                                <el-button
                                    ...
                                    :disabled="$hasBP('bnt.patient.remove') === false"
                                    title="删除"
                                />
                            </template>
                        </el-table-column>
            ...
            <el-table-column label="操作" align="center" fixed="right">
                <template slot-scope="scope">
                    <el-button
                        ...
                        :disabled="$hasBP('bnt.patient.add') === false"
                        title="添加"
                    />
                    <el-button
                        type="primary"
        ...
        <!-- 病人添加表 -->
        <el-dialog title="添加/修改" :visible.sync="dialogPatientVisible" width="40%">
            <el-form
                ref="dataForm"
                :model="patient"
                label-width="100px"
            ...
            >
                ...
                <el-form-item label="身份证号" prop="identifyCardNumber">
                    <el-input v-model="patient.identifyCardNumber" />
                </el-form-item>
                <el-form-item label="性别" prop="sex">
                    <el-radio-group v-model="patient.sex" size="mini">
                ...
                <el-form-item label="状况" prop="status">
                    <el-radio-group v-model="patient.status" size="mini">
                        <el-radio :label="true" border>已认证</el-radio>
                        <el-radio :label="false" border>未认证</el-radio>
                    </el-radio-group>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button
                    @click="dialogPatientVisible = false"
                    size="small"
                    icon="el-icon-refresh-right"
                    >取 消</el-button
                >
...
export default {
    ...
    methods: {
        ...
        fetchData(page = 1) {
            ...
            userApi.getPageListWithPatients(this.page, this.limit, this.searchObj).then(
                response => {
        ...
        removePatientById(id) {
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                // 点击确定，远程调用ajax
                return patientApi.removeById(id)
            }).then((response) => {
                this.fetchData(this.page)
                this.$message.success(response.message || '删除成功')
            }).catch(() => {
        ...
        editPatient(id) {
            ...
            patientApi.getById(id).then(response => {
                ...
                this.patient.birthdate = new Date(this.patient.birthdate).toLocaleString()
                this.patient.creationTime = new Date(this.patient.creationTime).toLocaleString()
            })
        },
        ...
        authPatient(id) {
            patientApi.authPatient(id).then(res => {
                this.$message({
                    message: "患者认证状态修改成功",
                    type: 'success'
                })
